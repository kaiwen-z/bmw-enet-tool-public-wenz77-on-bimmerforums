#!/usr/bin/env python3
"""
BMW F10 535i  –  Live Sensor Dashboard
HSFZ / UDS over ENET (TCP port 6801)
Sensor reads via UDS 0x2C DynamicallyDefineDataIdentifier → scratch DID 0xF300

DID sources: ISTA DATEN analysis
"""

import tkinter as tk
import socket, threading, struct, time, queue, math, csv, os, sys, subprocess, multiprocessing
import plot_bmw_log
...
#  DIDs sourced from public F-chassis DATEN/community analysis.
#  Scale factors marked † are estimated from community docs and should
#  be verified against known conditions (e.g. battery at rest ≈ 12.5V).
# ─────────────────────────────────────────────────────────
def _rpm(r):  return r * 0.25           # confirmed: 3374 raw → 843.5 RPM ✓
def _bat(r):  return r * 0.1            # confirmed: 147 raw → 14.70 V ✓
def _lp(r):   return r * 0.0145038     # confirmed: 6015 raw → 87.23 PSI ✓
def _hp(r):   return r * 0.0725        # confirmed: 10230 raw → 741.68 PSI ✓
def _clt(r):  return r * 0.5 - 3.5    # confirmed with offset correction ✓
def _oil(r):  return r * 0.0145675     # confirmed: 2732 raw → 39.80 PSI ✓
def _oilt(r): return r * 0.51275510    # confirmed: 196 raw → 100.50 °C ✓
def _bst(r):  return r * 0.00113310    # confirmed: 12985 raw → 14.71 PSI ✓
def _thr(r):  return r * 0.02437500    # confirmed: 112 raw → 2.73 % ✓
def _ivac(r): return r * 0.00056536    # confirmed: 24752 raw → 13.99 PSI ✓
def _vtec(r): return r * 0.10000000    # confirmed: 253 raw → 25.30 deg ✓
...
SENSORS = [
    ...
    ("Coolant Temp",         0x4300, 0x12,  1,  _clt,  "°C",    0,     130,   100,    115,   1),
    ("Oil Pressure",         0x586F, 0x12,  2,  _oil,  "PSI",   0,      90,    75,     85,   1),
    ("Engine Oil Temp",      0x4402, 0x12,  2,  _oilt, "°C",    0,     150,   120,    135,   1),
...
# ─────────────────────────────────────────────────────────
#  Colour palette  –  "Precision Dark" instrument aesthetic
# ─────────────────────────────────────────────────────────
BG        = "#0a0c10"
PANEL     = "#0f1219"
BORDER    = "#1c2030"
...
VIN_C     = "#f0f2ff"
META_C    = "#8898b8"
# Disabled sensor list row (shading when sensor is turned off)
DISABLED_ROW_BG = "#0a0d12"
DISABLED_ROW_FG = "#404858"
# Disabled gauge overlay text (subtle)
OVERLAY_TEXT = "#3d4555"
# Additional UI colours
TICK_MID         = "#4a5478"
OVERLAY_LABEL_DIM = "#5a6a88"
STRIP_BG         = "#0d1018"
BOX_OUTLINE      = "#2a3050"
BOX_TITLE        = "#8a9bc0"
ENTRY_BG         = "#1a1e2a"
BTN_BG           = "#1c2030"
BTN_ACTIVE_BG    = "#232a3a"
ACCENT_ACTIVE    = "#0060b0"
WHITE            = "white"
LOG_BG           = "#07090d"
BLACK            = "#000"
LOGGING_ACTIVE   = "#8b1a1a"
STIPPLE          = "gray50"
# Connect button background when disabled (e.g. during replay)
CBTN_BG_DISABLED = BTN_BG

TITLE_FONT = ("Segoe UI",  11, "bold")
LABEL_FONT = ("Segoe UI",  8)
SMALL_FONT = ("Segoe UI",  7)
MONO_FONT  = ("Courier New", 9)
MONO_BIG   = ("Courier New", 13, "bold")
VALUE_FONT = ("Courier New", 14, "bold")
...
class Dashboard(tk.Tk):
    ...
    def _replay_load(self):
        ...
        try:
            ...
            next(reader, None)
            self._replay_data = [r for r in reader if any(c.strip() for c in r)]
        except Exception as e:
            self._evt(f"Replay load failed: {e}", "err")
            return
        if not self._replay_data:
            self._evt("Log file has no data rows", "err")
            return
        # Pre-parse first/last timestamps for the time display
        self._replay_t0 = self._parse_replay_ts(
            self._replay_data[0][0] if self._replay_data[0] else "")
        self._replay_tN = self._parse_replay_ts(
            self._replay_data[-1][0] if self._replay_data[-1] else "")
